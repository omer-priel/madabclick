resource "aws_imagebuilder_image_recipe" "frontend" {
  ...
  component {
    component_arn = "arn:${var.aws_partition}:imagebuilder:${var.aws_region}:aws:component/aws-codedeploy-agent-linux/x.x.x"
  }
...
resource "aws_iam_role_policy_attachment" "prod_image_builder_EC2InstanceProfileForImageBuilder" {
  role       = aws_iam_role.prod_image_builder.id
  policy_arn = "arn:aws:iam::aws:policy/EC2InstanceProfileForImageBuilder"
}

resource "aws_iam_role_policy_attachment" "prod_image_builder_EC2InstanceProfileForImageBuilderECRContainerBuilds" {
  role       = aws_iam_role.prod_image_builder.id
  policy_arn = "arn:aws:iam::aws:policy/EC2InstanceProfileForImageBuilderECRContainerBuilds"
}

resource "aws_iam_role_policy_attachment" "prod_image_builder_AmazonSSMManagedInstanceCore" {
  role       = aws_iam_role.prod_image_builder.id
